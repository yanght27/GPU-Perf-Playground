...
usage() {
  cat <<'EOF'
Usage: bash environments/gpp-deepspeed-0.19.5.sh [--verify-only]

Run from the repository root. The default mode creates/updates the standalone
DeepSpeed 0.19.5 lane and runs basic package/import/CUDA checks.
EOF
}
...
lock = json.loads(Path("config/day0-lock.json").read_text())
environment = next(item for item in lock["environments"] if item["name"] == "gpp-deepspeed-0.19.5")
...
if not torch.cuda.is_available():
    raise SystemExit("CUDA is not available in gpp-deepspeed-0.19.5")
...
if y[0, 0].item() != 1024.0:
    raise SystemExit("DeepSpeed lane CUDA matmul returned the wrong value")
print("python_environment: gpp-deepspeed-0.19.5")
print("torch:", torch.__version__, "wheel_cuda:", torch.version.cuda)
print("deepspeed:", deepspeed.__version__, "triton:", triton.__version__)
print("gpu:", properties.name, "sm:", capability, "memory_mib:", memory_mib)
print("matmul_value:", y[0, 0].item())
print("gpp_deepspeed: PASS")
PY
