...
usage() {
  cat <<'EOF'
Usage: bash environments/gpp-swift-4.4.3.sh [--verify-only]

Run from the repository root. The default mode creates/updates the isolated
ms-swift v4.4.3 commit-compatible lane and runs package/import/CUDA checks.
EOF
}
...
source = environment["ms_swift"]
print("\t".join((
    environment["python"], packages["torch"], packages["transformers"],
    packages["triton"], packages["deepspeed"], packages["vllm"],
    packages["ms-swift"], source["repository"], source["commit"],
)))
PY
)"
IFS=$'\t' read -r PYTHON_VERSION TORCH_VERSION TRANSFORMERS_VERSION \
  TRITON_VERSION DEEPSPEED_VERSION VLLM_VERSION MS_SWIFT_DISTRIBUTION_VERSION \
  MS_SWIFT_REPOSITORY MS_SWIFT_COMMIT <<<"${lock_values}"
...
if [[ "${MODE}" == "install" ]]; then
  ...
  run_env python -I -m pip install \
    ...
    "vllm==${VLLM_VERSION}" \
    "ms-swift @ git+${MS_SWIFT_REPOSITORY}@${MS_SWIFT_COMMIT}"
fi
...
lock = json.loads(Path("config/day0-lock.json").read_text())
environment = next(item for item in lock["environments"] if item["name"] == "gpp-swift-4.4.3")
...
direct_url = json.loads(metadata.distribution("ms-swift").read_text("direct_url.json"))
observed_commit = direct_url.get("vcs_info", {}).get("commit_id")
expected_commit = source["commit"]
if observed_commit != expected_commit:
    raise SystemExit(f"ms-swift commit: expected {expected_commit}, observed {observed_commit}")
if not torch.cuda.is_available():
    raise SystemExit("CUDA is not available in gpp-swift-4.4.3")
...
print("python_environment: gpp-swift-4.4.3")
...
print("ms-swift_distribution:", metadata.version("ms-swift"))
print("ms-swift_commit:", observed_commit)
print("gpu:", properties.name, "sm:", capability, "memory_mib:", memory_mib)
print("gpp_swift: PASS")
PY
