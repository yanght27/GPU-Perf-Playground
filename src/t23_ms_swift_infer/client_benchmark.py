"""T23 ms-swift 推理客户端：向 OpenAI 兼容服务发请求，记录 TTFT/ITL/TPOT/吞吐。

官方依据：
- ms-swift README / docs 的 `swift deploy`（S08a）；
- ms-swift 使用 vLLM/SGLang 等底层引擎，对外提供 OpenAI 兼容 API（S08a）。
...
用法：
    ...
    # 不连服务器，只检查脚本和参数是否可运行：
    conda run --no-capture-output -n gpp-core python -I \
        src/t23_ms_swift_infer/client_benchmark.py --dry-run
"""
...
def chat_completion_stream(base_url, prompt, max_tokens):
    """向 ms-swift 发 streaming ChatCompletion，返回耗时指标和文本。"""
...
def run(base_url, max_tokens, prompts):
    print(f"[t23] base_url={base_url} max_tokens={max_tokens}")
    models = requests.get(base_url.rstrip("/") + "/v1/models", timeout=10)
    models.raise_for_status()
    print(f"[t23] /v1/models -> {models.json()['data'][0]['id']}")

    results = []
    for prompt in prompts:
        r = chat_completion_stream(base_url, prompt, max_tokens)
        results.append(r)
        print(f"[t23] prompt={r['prompt']} new_tokens={r['new_tokens']} "
        ...
        print(f"[t23]   response={r['text']!r}")

    print("[t23] SUMMARY")
    for r in results:
        print(f"[t23]   {r['prompt']}: ttft={r['ttft_s']:.4f} itl={r['avg_itl_s']:.4f} "
              f"tpot={r['tpot_s']:.4f} throughput={r['throughput_tok_s']:.2f} "
              f"new_tokens={r['new_tokens']} response={r['text']!r}")


def main():
    parser = argparse.ArgumentParser(description="T23 ms-swift OpenAI-compatible client benchmark")
    parser.add_argument("--base-url", default="http://localhost:8000", help="ms-swift server base URL")
    ...
    if args.dry_run:
        print("[t23] dry-run: no server connection")
        for p in prompts:
            print(f"[t23]   would send prompt={p['name']} max_tokens={args.max_tokens}")
        return
