"""T21 SGLang Serving 客户端：向 OpenAI 兼容服务发请求，记录 TTFT/ITL/TPOT/吞吐。

官方依据：
- SGLang Quickstart（S06a，https://raw.githubusercontent.com/sgl-project/sglang/v0.5.17/docs/docs/get-started/quickstart.mdx）；
- SGLang OpenAI-compatible API（S06b）。
...
用法：
    ...
    # 不连服务器，只检查脚本和参数是否可运行：
    conda run --no-capture-output -n gpp-core python -I \
        src/t21_sglang_serving/client_benchmark.py --dry-run
"""
...
def chat_completion_stream(base_url, prompt, max_tokens):
    """向 SGLang 发 streaming ChatCompletion，返回耗时指标和文本。"""
...
def run(base_url, max_tokens, prompts):
    print(f"[t21] base_url={base_url} max_tokens={max_tokens}")
    models = requests.get(base_url.rstrip("/") + "/v1/models", timeout=10)
    models.raise_for_status()
    print(f"[t21] /v1/models -> {models.json()['data'][0]['id']}")

    results = []
    for prompt in prompts:
        r = chat_completion_stream(base_url, prompt, max_tokens)
        results.append(r)
        print(f"[t21] prompt={r['prompt']} new_tokens={r['new_tokens']} "
        ...
        print(f"[t21]   response={r['text']!r}")

    print("[t21] SUMMARY")
    for r in results:
        print(f"[t21]   {r['prompt']}: ttft={r['ttft_s']:.4f} itl={r['avg_itl_s']:.4f} "
              f"tpot={r['tpot_s']:.4f} throughput={r['throughput_tok_s']:.2f} "
              f"new_tokens={r['new_tokens']} response={r['text']!r}")


def main():
    parser = argparse.ArgumentParser(description="T21 SGLang OpenAI-compatible client benchmark")
    ...
    if args.dry_run:
        print("[t21] dry-run: no server connection")
        for p in prompts:
            print(f"[t21]   would send prompt={p['name']} max_tokens={args.max_tokens}")
        return
