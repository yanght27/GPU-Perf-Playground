"""T11 Reduction 共享内存规约 —— 路径 4：cuTile Python（ct.sum）。

官方依据：cuTile 官方 reduction 测试 test_reduction.py 的 ct.sum/axis 写法（台账 S03k）。
结构：把 1 维数组垫成 (rows, TILE=128) 的 2 维，每 block 用 ct.sum(axis=1) 归约一行；
host 用 fp64 汇总 rows 个部分和。补齐的 0 是加法单位元，不改变总和。
"""
...
def run(N, seed=0):
    ...
    summarize_error(cp.asarray(out), cp.asarray(ref), f"cutile_t11_{N}", tolerance=TOL)
