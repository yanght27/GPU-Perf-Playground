"""T12 Reduction Warp Shuffle —— 路径 1：PyTorch（语义与 fp64 黄金参考）。

官方依据：torch.Tensor.sum / Tensor.double（PyTorch 2.13 文档，台账 S15c）。
本 Ticket 关注 warp 级 shuffle 规约；PyTorch 高层 API 不暴露底层 shuffle 细节，
因此该路径定位为“同一语义的正确性参考 + fp64 黄金参考”，不参与机制对比。
"""
...
def run(N, seed=0):
    ...
    summarize_error(out, ref, f"pytorch_t12_{N}", tolerance=TOL)
