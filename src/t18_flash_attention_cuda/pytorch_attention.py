"""T18 FlashAttention CUDA 版 —— 路径 1：PyTorch SDPA 黄金参考。

官方依据：PyTorch `F.scaled_dot_product_attention`（台账 S15e）。
本路径为 CUDA 版提供 causal/non-causal 与 512/1024/2048 的黄金参考；同时给 fp64
eager 参考，证明 SDPA 自身可信。
"""
...
def run_case(B, H, N, D, causal):
    torch.manual_seed(0)
    q = torch.randn(B, H, N, D, device="cuda", dtype=torch.float32) * 0.2
    k = torch.randn(B, H, N, D, device="cuda", dtype=torch.float32) * 0.2
    v = torch.randn(B, H, N, D, device="cuda", dtype=torch.float32) * 0.2
    ...
    summarize_error(out, ref64, f"pytorch_sdpa_t18_B{B}_H{H}_N{N}_D{D}_causal{causal}", tolerance=TOL)
